...
const schemaAnalyzer = {
  ...
  run: function(confFile, schemaConfigFile, generator){
    ...
    read.on('line', (line) => {
      ...
      if(level == 0){
        ...
        if(fieldDef.length != 3){
          throw new Error(this.confFile + '中，本行(' + lineNo + ')表示字段定义，格式为:fieldType|fieldDefault|fieldNotNull')
        }
      ...
      fs.writeFile(this.schemaConfigFile, 'module.exports = ' + JSON.stringify(this.config), function (err) {
          if (err) throw err
          log.info('\n\t' + this.schemaConfigFile + ' is saved!')
      })
